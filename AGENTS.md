...
Deploying a version is [DEPLOY.md](DEPLOY.md) — checks, this app's own changelog, commit, push, then **one** ship command chosen by a gate: `npm run ota` when the fingerprint still matches every shipped store build *and* the batch contains nothing an OTA cannot take back, `npm run provision` otherwise. **`ota` publishes to every installed phone and tags; `provision` publishes nothing.** `ota` is the default for a `src/`-only batch: only a fingerprint mismatch, a lossy on-device migration, or the user asking for a build routes to `provision`. A protocol change does not — the desktop ships alongside the phone. Anything genuinely unshippable is a stop on both paths, not a provision. Never run `release` or `rollback`, never set `OTA_SKIP_RUNTIME_CHECK=1`, never run `eas` beyond the read-only `build:list` the gate uses, never create or push a tag by hand. Versions and the README badge are written by `scripts/provision.js` and `scripts/ota.js`; never write them by hand.
...
- **A new screen a notification should be able to open needs `DEEPLINK_ROUTES`** in `src/lib/tunnel/protocol.ts` **and the desktop's mirrored copy**. It is an allowlist on both ends: the desktop refuses a link naming anything else (which is how the model learns the real page list), and this app ignores one, so a screen missing from it is simply unreachable by tap.
